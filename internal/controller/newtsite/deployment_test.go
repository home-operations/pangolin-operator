...
const (
	containerName = newtContainerName
	debugLevel    = "DEBUG"

	testSiteName    = "my-site"
	testNamespace   = "default"
	testCredentials = "my-site-newt-credentials" //nolint:gosec // secret name fixture, not a real credential
	testKey         = "key"
)
...
func TestBuildDeployment_Defaults(t *testing.T) {
	site := newTestSite()
	d := buildDeployment(site, testCredentials, "test-ns")

	if d.Name != testSiteName {
...
func TestBuildDeployment_EnvVarsFromSecret(t *testing.T) {
	...
	for _, e := range c.Env {
		if e.ValueFrom != nil && e.ValueFrom.SecretKeyRef != nil {
			keys[e.Name] = true
			if e.ValueFrom.SecretKeyRef.Name != testCredentials {
				t.Errorf("env %s references wrong secret: %q", e.Name, e.ValueFrom.SecretKeyRef.Name)
			}
		}
	}
	for _, required := range []string{envPangolinEndpoint, envNewtID, envNewtSecret} {
		if !keys[required] {
...
func TestBuildDeployment_Labels(t *testing.T) {
	...
	if labels["app.kubernetes.io/instance"] != testSiteName {
...
func TestBuildDeployment_OptionalEnvVars(t *testing.T) {
	tests := []struct {
		...
		{envDNS, func(s *pangolinv1alpha1.NewtSite) { s.Spec.Newt.DNS = "1.1.1.1" }, envDNS, "1.1.1.1"},
...
func TestBuildDeployment_DefaultInterfaceOmitted(t *testing.T) {
	site := newTestSite()
	site.Spec.Newt.Interface = newtContainerName // default value
	d := buildDeployment(site, "creds", "test-ns")
...
func TestBuildDeployment_ExtraEnvOverrides(t *testing.T) {
	site := newTestSite()
	site.Spec.Newt.ExtraEnv = []corev1.EnvVar{{Name: envLogLevel, Value: debugLevel}}
	d := buildDeployment(site, "creds", "test-ns")
	envs := d.Spec.Template.Spec.Containers[0].Env
	// The last LOG_LEVEL should be the override.
	last := ""
	for _, e := range envs {
		if e.Name == envLogLevel {
			last = e.Value
		}
	}
	if last != debugLevel {
...
func TestBuildDeployment_SchedulingFields(t *testing.T) {
	...
	site.Spec.Newt.Tolerations = []corev1.Toleration{{Key: testKey, Operator: corev1.TolerationOpExists}}
...
func TestBuildDeployment_LogLevel(t *testing.T) {
	...
	d := buildDeployment(site, "creds", "test-ns")
	env, ok := findEnv(d.Spec.Template.Spec.Containers[0].Env, envLogLevel)
	if !ok {
